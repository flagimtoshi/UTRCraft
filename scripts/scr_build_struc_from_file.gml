...
structure_generating=1 // besagt das structuren gebaut werden (NEU)
...
//if (wallins = i){ // Der Reparatur Script der mir meine Träume eine gute Person zu sein erfüllt
    ...
    show_world_around_point(24, spr_solid, xxx+xx, yyy+yy);
    //if place_meeting(xxx+xx,yyy+yy,obj_solid){  // Checkt Kollision
    //instance_create(xxx+xx,yyy+yy,obj_nova_des);} //Wenn Kollision dann boom
    inst = instance_place(xxx+xx,yyy+yy, obj_solid);
    if inst != noone
    {    
...
//}else{
    ...
    ioo_=instance_create(xxx+xx,yyy+yy,BSTrans); // setzt Block der geholt wird
    scr_special_data_load(ioo_, i);
...
structure_generating=0; //besagt das wir fertig sind (neu)
